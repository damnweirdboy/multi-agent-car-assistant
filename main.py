...
from agents import RouterAgent, RetrieverAgent, AdvisorAgent, MonitorAgent
...
from agents.monitor_agent import MonitoringResult
...
def _print_monitor(monitor_res: MonitoringResult) -> None:
    print(f"\n┌{'─' * (WIDTH - 2)}┐")
    print(f"│  STEP 4 — MONITOR AGENT{' ' * (WIDTH - 26)}│")
    print(f"├{'─' * (WIDTH - 2)}┤")
    wrapped = textwrap.wrap(monitor_res.feedback, width=WIDTH - 6)
    for line in wrapped:
        print(f"│   {line:<{WIDTH - 5}}│")
    print(f"└{'─' * (WIDTH - 2)}┘\n")
...
def _run_query(
    ...
    monitor_res: MonitoringResult = monitor.check_response(question, advice.answer, context)
    _print_monitor(monitor_res)
...
def main() -> None:
    ...
    if args.demo:
        ...
        for question in DEMO_QUESTIONS:
            _run_query(question, router, retriever, advisor, monitor)
        print(f"{'═' * WIDTH}")
        print("  ✅ Demo complete.")
        print(f"{'═' * WIDTH}\n")
    else:
